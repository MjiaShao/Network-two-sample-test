...
for mf = 1:length(MotifNameall)
    for k = 1:length(N)
        for l = 1:length(N)
            ...
            cover = readtable(strcat("./result/MC_t_cover_edge_",string(m),"_",string(n),"_",MotifName,"_",GraphonName1,"_",GraphonName2,"_mn.csv"));
            cover = table2array(cover);
            coveragemean(k,l) = nanmean(cover);
            coveragestd(k,l) = nanstd(cover,0);
            cover = readtable(strcat("./result/MC_t_cover_norm_",string(m),"_",string(n),"_",MotifName,"_",GraphonName1,"_",GraphonName2,"_mn.csv"));
            cover = table2array(cover);
